...
test.describe("viewport + nav + form QA", () => {
  for (const vp of viewports) {
    test(`no console errors — ${vp.name} (${vp.width}px)`, async ({ page }) => {
      ...
      await page.locator('.inquiry-form select[name="preferred-sport"]').selectOption({ index: 0 });
      await page.locator('.inquiry-form select[name="travel-type"]').selectOption({ index: 1 });
      await page.locator('.inquiry-form input[name="destination-interest"]').fill("Spain");
      await page.locator('.inquiry-form input[name="group-size"]').fill("4");
      await page.locator('.inquiry-form input[name="travel-window"]').fill("June");
